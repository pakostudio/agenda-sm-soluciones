...
export async function GET(request: Request) {
  ...
  if (!token) return NextResponse.json({ connected: false, configured: true }, { status: 401 });
  ...
  if (error || !userData.user) return NextResponse.json({ connected: false, configured: true }, { status: 401 });
